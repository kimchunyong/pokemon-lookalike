/**
 * PokeAPI에서 3세대 포켓몬(252~386) 메타데이터를 가져와 JSON으로 저장한다.
 *
 * 사용법: node scripts/fetch-gen3-pokemon.js
 * 결과물: data/pokemon-gen3.json
 */
...
const OUTPUT_PATH = resolve(__dirname, '..', 'data', 'pokemon-gen3.json')
...
function getKoreanFlavorText(species) {
  ...
  const entry = entries.find((e) => e.version?.name === 'ruby') ?? entries.find((e) => e.version?.name === 'emerald') ?? entries[0]
...
async function main() {
  ...
  for (let id = GEN3_START; id <= GEN3_END; id++) {
    try {
      ...
      process.stdout.write(`\r[${id - GEN3_START + 1}/${GEN3_END - GEN3_START + 1}] #${id} ${name}`)
    } catch (err) {
      console.error(`\n#${id} FAIL:`, err.message)
    }
  }
