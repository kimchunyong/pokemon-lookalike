/**
 * PokeAPI에서 4세대 포켓몬(387~493) 메타데이터를 가져와 JSON으로 저장한다.
 *
 * 사용법: node scripts/fetch-gen4-pokemon.js
 * 결과물: data/pokemon-gen4.json
 */
...
const OUTPUT_PATH = resolve(__dirname, '..', 'data', 'pokemon-gen4.json')
...
function getKoreanFlavorText(species) {
  ...
  const entry =
    entries.find((e) => e.version?.name === 'diamond') ??
    entries.find((e) => e.version?.name === 'pearl') ??
    entries[0]
...
async function main() {
  ...
  for (let id = GEN4_START; id <= GEN4_END; id++) {
    try {
      ...
      process.stdout.write(`\r[${id - GEN4_START + 1}/${GEN4_END - GEN4_START + 1}] #${id} ${name}`)
    } catch (err) {
      console.error(`\n#${id} FAIL:`, err.message)
    }
  }
